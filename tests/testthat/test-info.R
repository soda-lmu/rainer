...
test_that("header returns correct structure for datasets", {
  ...
  # Test the function
  result <- header()
  # Check that column names are correctly identified via new structure
  expect_equal(names(result[["df1"]]$columns), c("col1", "col2"))
  expect_equal(names(result[["df2"]]$columns), c("name", "age"))
  # Check column types
  expect_equal(result[["df1"]]$columns$col1$type, "integer")
  expect_equal(result[["df1"]]$columns$col2$type, "character")
  expect_equal(result[["df2"]]$columns$name$type, "character")
  expect_equal(result[["df2"]]$columns$age$type, "numeric")
  # Check has_na is present for all columns
  expect_false(result[["df1"]]$columns$col1$has_na)
  expect_false(result[["df1"]]$columns$col2$has_na)
...
test_that("environment_info returns correctly structured information", {
  ...
  expect_equal(result_with_error$datastructure$test_df$columns$x$type, "integer")
})
